...
use ethabi::{ethereum_types::U256, FixedBytes, ParamType, Token};
...
fn main() {
    ...
    let input = ethabi::decode_whole(
        &[
            ParamType::FixedBytes(32), // request root
    ...
    let request_root: FixedBytes = input[0].clone().into_fixed_bytes().unwrap();
